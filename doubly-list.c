...
list *add_to_end(int d, list *h){
    /*
        ...
        El miembro prev del nuevo head creado apunta ahora
        al elemento último de la lista. El nuevo elemento
    ...
    */
    ...
    head->next = NULL;
    if (h!=NULL){
        // Si la lista tiene elementos, buscamos el último elemento
        // agregado
        end = h;
        while (end->next != NULL){
            end = end->next;
    ...
    // Implicítamente asumimos que h siempre apunta al primer elemento de la lista
    // luego, por coherencia retornamos el primer elemento de h, ya con el nuevo
    // elemento agregado
    if (h!=NULL){
        start = head;
